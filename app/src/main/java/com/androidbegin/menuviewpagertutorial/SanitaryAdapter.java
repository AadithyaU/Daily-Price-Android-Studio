...
public class SanitaryAdapter extends BaseAdapter {
	...
	private static List<WorldPopulation> sanitarylist = null;
	...
	public SanitaryAdapter(Context context,
			List<WorldPopulation> sanitarylist) {
		this.context = context;
		this.sanitarylist = sanitarylist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(sanitarylist);
	...
	@Override
	public int getCount() {
		return sanitarylist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return sanitarylist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=SanitaryAdapter.arraylist;
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(sanitarylist.get(position).getPrice());
		holder.Product.setText(sanitarylist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(sanitarylist.get(position).getPicture(),
				holder.Picture);

		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = sanitarylist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != SanitaryAdapter.getProductQuantity(selectedProduct))){
      		  SanitaryAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(SanitaryAdapter.getProductQuantity(selectedProduct) != 0) {
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((SanitaryAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (SanitaryAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (SanitaryAdapter.getProductQuantity(selectedProduct) != 0) && (m != SanitaryAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						SanitaryAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 SanitaryAdapter.setQuantity(selectedProduct, 0);
