...
public class SaltsugarjaggeryAdapter extends BaseAdapter {
	...
	public SaltsugarjaggeryAdapter(Context context,
			List<WorldPopulation> saltsugarjaggerylist) {
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		imageLoader.DisplayImage(saltsugarjaggerylist.get(position).getPicture(),
				holder.Picture);
        ...
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != SaltsugarjaggeryAdapter.getProductQuantity(selectedProduct))){
      		  SaltsugarjaggeryAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((SaltsugarjaggeryAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (SaltsugarjaggeryAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (SaltsugarjaggeryAdapter.getProductQuantity(selectedProduct) != 0) && (m != SaltsugarjaggeryAdapter.getProductQuantity(selectedProduct)))
