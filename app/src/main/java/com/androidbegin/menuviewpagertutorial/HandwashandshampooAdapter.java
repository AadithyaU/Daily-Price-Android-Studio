...
public class HandwashandshampooAdapter extends BaseAdapter {
	...
	private static List<WorldPopulation> handwashandshampoolist = null;
	...
	public HandwashandshampooAdapter(Context context,
			List<WorldPopulation> handwashandshampoolist) {
		this.context = context;
		this.handwashandshampoolist = handwashandshampoolist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(handwashandshampoolist);
	...
	@Override
	public int getCount() {
		return handwashandshampoolist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return handwashandshampoolist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=HandwashandshampooAdapter.arraylist;
			
		//}
		
		return handwashandshampoolist;
	}
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(handwashandshampoolist.get(position).getPrice());
		holder.Product.setText(handwashandshampoolist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(handwashandshampoolist.get(position).getPicture(),
				holder.Picture);
		// Listen for ListView Item Click

		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = handwashandshampoolist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != HandwashandshampooAdapter.getProductQuantity(selectedProduct))){
      		  HandwashandshampooAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(HandwashandshampooAdapter.getProductQuantity(selectedProduct) != 0) {
        ...
        button1.setOnClickListener(new OnClickListener() {  
		 ...
		 public boolean onMenuItemClick(android.view.MenuItem item) {  
	         ...
	         np.setValue(HandwashandshampooAdapter.getProductQuantity(selectedProduct));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((HandwashandshampooAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (HandwashandshampooAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (HandwashandshampooAdapter.getProductQuantity(selectedProduct) != 0) && (m != HandwashandshampooAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						HandwashandshampooAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 HandwashandshampooAdapter.setQuantity(selectedProduct, 0);
    ...
    public void remove(WorldPopulation object) {
        handwashandshampoolist.remove(object);
	...
	public void filter(String charText) {
		...
		handwashandshampoolist.clear();
		if (charText.length() == 0) {
			handwashandshampoolist.addAll(arraylist);
		} else {
			for (WorldPopulation wp : arraylist) {
				if (wp.getProduct().toLowerCase(Locale.getDefault()).contains(charText)) {
					handwashandshampoolist.add(wp);
				}
			}
		}
		notifyDataSetChanged();
