...
public class DrinksandjuicesAdapter extends BaseAdapter {
	...
	public DrinksandjuicesAdapter(Context context,
			List<WorldPopulation> drinksandjuiceslist) {
		this.context = context;
		this.drinksandjuiceslist = drinksandjuiceslist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(drinksandjuiceslist);
	...
	@Override
	public int getCount() {
		return drinksandjuiceslist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return drinksandjuiceslist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=DrinksandjuicesAdapter.arraylist;
			
		//}
		
		return drinksandjuiceslist;
	}
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(drinksandjuiceslist.get(position).getPrice());
		holder.Product.setText(drinksandjuiceslist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(drinksandjuiceslist.get(position).getPicture(),
				holder.Picture);
		// Listen for ListView Item Click
	
		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = drinksandjuiceslist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != DrinksandjuicesAdapter.getProductQuantity(selectedProduct))){
      		  DrinksandjuicesAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(DrinksandjuicesAdapter.getProductQuantity(selectedProduct) != 0) {
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((DrinksandjuicesAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (DrinksandjuicesAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (DrinksandjuicesAdapter.getProductQuantity(selectedProduct) != 0) && (m != DrinksandjuicesAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						DrinksandjuicesAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 DrinksandjuicesAdapter.setQuantity(selectedProduct, 0);
    ...
    public void remove(WorldPopulation object) {
        drinksandjuiceslist.remove(object);
	...
	public void filter(String charText) {
		...
		drinksandjuiceslist.clear();
		if (charText.length() == 0) {
			drinksandjuiceslist.addAll(arraylist);
		} else {
			for (WorldPopulation wp : arraylist) {
				if (wp.getProduct().toLowerCase(Locale.getDefault()).contains(charText)) {
					drinksandjuiceslist.add(wp);
				}
			}
		}
		notifyDataSetChanged();
