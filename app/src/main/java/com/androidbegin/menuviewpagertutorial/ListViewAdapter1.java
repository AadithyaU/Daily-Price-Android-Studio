...
public class ListViewAdapter1 extends BaseAdapter {
	...
	private static List<WorldPopulation> snackslist = null;
	...
	public ListViewAdapter1(Context context,
			List<WorldPopulation> snackslist) {
		this.context = context;
		this.snackslist = snackslist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(snackslist);
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=ListViewAdapter1.arraylist;
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(snackslist.get(position).getPrice());
		holder.Product.setText(snackslist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(snackslist.get(position).getPicture(),
				holder.Picture);
		// Listen for ListView Item Click

		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = snackslist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != ListViewAdapter1.getProductQuantity(selectedProduct))){
      		  ListViewAdapter1.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(ListViewAdapter1.getProductQuantity(selectedProduct) != 0) {
        ...
        button1.setOnClickListener(new OnClickListener() {  
		 ...
		 public boolean onMenuItemClick(android.view.MenuItem item) {  
	         ...
	         np.setValue(ListViewAdapter1.getProductQuantity(selectedProduct));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((ListViewAdapter1.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (ListViewAdapter1.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (ListViewAdapter1.getProductQuantity(selectedProduct) != 0) && (m != ListViewAdapter1.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						ListViewAdapter1.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 ListViewAdapter1.setQuantity(selectedProduct, 0);
