...
public class BreakfastAdapter extends BaseAdapter {
	...
	private static List<WorldPopulation> breakfastlist = null;
	...
	public BreakfastAdapter(Context context,
			List<WorldPopulation> breakfastlist) {
		this.context = context;
		this.breakfastlist = breakfastlist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(breakfastlist);
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=BreakfastAdapter.arraylist;
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(breakfastlist.get(position).getPrice());
		holder.Product.setText(breakfastlist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(breakfastlist.get(position).getPicture(),
				holder.Picture);
		
		
		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = breakfastlist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != BreakfastAdapter.getProductQuantity(selectedProduct))){
      		  BreakfastAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(BreakfastAdapter.getProductQuantity(selectedProduct) != 0) {
        ...
        button1.setOnClickListener(new OnClickListener() {  
		 ...
		 public boolean onMenuItemClick(android.view.MenuItem item) {  
	         ...
	         np.setValue(BreakfastAdapter.getProductQuantity(selectedProduct));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((BreakfastAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (BreakfastAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (BreakfastAdapter.getProductQuantity(selectedProduct) != 0) && (m != BreakfastAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						BreakfastAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 BreakfastAdapter.setQuantity(selectedProduct, 0);
