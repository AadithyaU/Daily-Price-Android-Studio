...
public class RefrigeratedAdapter extends BaseAdapter {
	...
	private static List<WorldPopulation> refrigeratedlist = null;
	...
	public RefrigeratedAdapter(Context context,
			List<WorldPopulation> refrigeratedlist) {
		this.context = context;
		this.refrigeratedlist = refrigeratedlist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(refrigeratedlist);
	...
	@Override
	public int getCount() {
		return refrigeratedlist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return refrigeratedlist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=RefrigeratedAdapter.arraylist;
			
		//}
		
		return refrigeratedlist;
	}
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(refrigeratedlist.get(position).getPrice());
		holder.Product.setText(refrigeratedlist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(refrigeratedlist.get(position).getPicture(),
				holder.Picture);
		// Listen for ListView Item Click

		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = refrigeratedlist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != RefrigeratedAdapter.getProductQuantity(selectedProduct))){
      		  RefrigeratedAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(RefrigeratedAdapter.getProductQuantity(selectedProduct) != 0) {
        ...
        button1.setOnClickListener(new OnClickListener() {  
		 ...
		 public boolean onMenuItemClick(android.view.MenuItem item) {  
	         ...
	         np.setValue(RefrigeratedAdapter.getProductQuantity(selectedProduct));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((RefrigeratedAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (RefrigeratedAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (RefrigeratedAdapter.getProductQuantity(selectedProduct) != 0) && (m != RefrigeratedAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						RefrigeratedAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 RefrigeratedAdapter.setQuantity(selectedProduct, 0);
    ...
    public void remove(WorldPopulation object) {
        refrigeratedlist.remove(object);
	...
	public void filter(String charText) {
		...
		refrigeratedlist.clear();
		if (charText.length() == 0) {
			refrigeratedlist.addAll(arraylist);
		} else {
			for (WorldPopulation wp : arraylist) {
				if (wp.getProduct().toLowerCase(Locale.getDefault()).contains(charText)) {
					refrigeratedlist.add(wp);
				}
			}
		}
		notifyDataSetChanged();
