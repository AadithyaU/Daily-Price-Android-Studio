...
public class TeaandcoffeeAdapter extends BaseAdapter {
	...
	private static List<WorldPopulation> teaandcoffeelist = null;
	...
	public TeaandcoffeeAdapter(Context context,
			List<WorldPopulation> teaandcoffeelist) {
		this.context = context;
		this.teaandcoffeelist = teaandcoffeelist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(teaandcoffeelist);
	...
	@Override
	public int getCount() {
		return teaandcoffeelist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return teaandcoffeelist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=TeaandcoffeeAdapter.arraylist;
			
		//}
		
		return teaandcoffeelist;
	}
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(teaandcoffeelist.get(position).getPrice());
		holder.Product.setText(teaandcoffeelist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(teaandcoffeelist.get(position).getPicture(),
				holder.Picture);

		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = teaandcoffeelist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != TeaandcoffeeAdapter.getProductQuantity(selectedProduct))){
      		  TeaandcoffeeAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(TeaandcoffeeAdapter.getProductQuantity(selectedProduct) != 0) {
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((TeaandcoffeeAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (TeaandcoffeeAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (TeaandcoffeeAdapter.getProductQuantity(selectedProduct) != 0) && (m != TeaandcoffeeAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						TeaandcoffeeAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 TeaandcoffeeAdapter.setQuantity(selectedProduct, 0);
    ...
    public void remove(WorldPopulation object) {
        teaandcoffeelist.remove(object);
	...
	public void filter(String charText) {
		...
		teaandcoffeelist.clear();
		if (charText.length() == 0) {
			teaandcoffeelist.addAll(arraylist);
		} else {
			for (WorldPopulation wp : arraylist) {
				if (wp.getProduct().toLowerCase(Locale.getDefault()).contains(charText)) {
					teaandcoffeelist.add(wp);
				}
			}
		}
		notifyDataSetChanged();
