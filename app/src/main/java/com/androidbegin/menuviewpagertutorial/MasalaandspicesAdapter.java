...
public class MasalaandspicesAdapter extends BaseAdapter {
	...
	public MasalaandspicesAdapter(Context context,
			List<WorldPopulation> masalaandspiceslist) {
		this.context = context;
		this.masalaandspiceslist = masalaandspiceslist;
		inflater = LayoutInflater.from(context);
		this.arraylist = new ArrayList<WorldPopulation>();
		this.arraylist.addAll(masalaandspiceslist);
	...
	@Override
	public int getCount() {
		return masalaandspiceslist.size();
	}

	@Override
	public WorldPopulation getItem(int position) {
		return masalaandspiceslist.get(position);
	}
	...
	public static List<WorldPopulation> getCatalog(Resources res){
		...
		//	catalog=MasalaandspicesAdapter.arraylist;
			
		//}
		
		return masalaandspiceslist;
	}
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		holder.Price.setText(masalaandspiceslist.get(position).getPrice());
		holder.Product.setText(masalaandspiceslist.get(position).getProduct());
		// Set the results into ImageView
		imageLoader.DisplayImage(masalaandspiceslist.get(position).getPicture(),
				holder.Picture);
		
		final View temp = view;
		final ImageButton button1 = (ImageButton) view.findViewById(R.id.overflow);  
        final WorldPopulation selectedProduct = masalaandspiceslist.get(position);
        for(WorldPopulation p : mCartList) {
      	  if(p.getCategory().equals(selectedProduct.getCategory()) && p.getProduct().equals(selectedProduct.getProduct())  && (ShoppingCartHelper.getProductQuantity(p) != MasalaandspicesAdapter.getProductQuantity(selectedProduct))){
      		  MasalaandspicesAdapter.setQuantity(selectedProduct, ShoppingCartHelper.getProductQuantity(p));
        ...
        if(MasalaandspicesAdapter.getProductQuantity(selectedProduct) != 0) {
        ...
        button1.setOnClickListener(new OnClickListener() {  
		 ...
		 public boolean onMenuItemClick(android.view.MenuItem item) {  
	         ...
	         np.setValue(MasalaandspicesAdapter.getProductQuantity(selectedProduct));
	       	...
	       	.setPositiveButton("Set",new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog,int id) {
					final int m = np.getValue();
		        	 int flag = 0;
		        	 if((MasalaandspicesAdapter.getProductQuantity(selectedProduct) != 0) && m == 0) 
			      			Toast.makeText(context, "Removed from Cart Successfully", 500).show();
			      		else if (m != 0 && (MasalaandspicesAdapter.getProductQuantity(selectedProduct) == 0)) {
			      			Toast.makeText(context, "Added to Cart Successfully", 500).show(); 
			      				temp.setBackgroundColor(0x5500cc99);
			      			}
			      		else if (m!=0 && (MasalaandspicesAdapter.getProductQuantity(selectedProduct) != 0) && (m != MasalaandspicesAdapter.getProductQuantity(selectedProduct)))
			      			Toast.makeText(context, "Changed the Quantity Successfully", 500).show();
						ShoppingCartHelper.setQuantity(selectedProduct, m);
						MasalaandspicesAdapter.setQuantity(selectedProduct, m);
		              dialog.dismiss();	
				}
			});    
	         alert.show();
          ...
          popup.show();//showing popup menu  
    		 ...
    		 public boolean onMenuItemClick(android.view.MenuItem item) {  
    	         ...
    	         alert.show();
    				 ...
    				 MasalaandspicesAdapter.setQuantity(selectedProduct, 0);
    ...
    public void remove(WorldPopulation object) {
        masalaandspiceslist.remove(object);
	...
	public void filter(String charText) {
		...
		masalaandspiceslist.clear();
		if (charText.length() == 0) {
			masalaandspiceslist.addAll(arraylist);
		} else {
			for (WorldPopulation wp : arraylist) {
				if (wp.getProduct().toLowerCase(Locale.getDefault()).contains(charText)) {
					masalaandspiceslist.add(wp);
				}
			}
		}
		notifyDataSetChanged();
